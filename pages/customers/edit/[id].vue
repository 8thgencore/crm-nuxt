...
import { COLLECTION_CUSTOMERS, APPWRITE_DATABASE_ID, APPWRITE_STORAGE_ID } from '~/app.constants'
...
const { data, isSuccess } = useQuery({
    ...
    queryFn: () => DB.getDocument(APPWRITE_DATABASE_ID, COLLECTION_CUSTOMERS, customerId),
})
...
const { mutate, isPending } = useMutation({
    ...
    mutationFn: (data: ICustomerFormState) =>
        DB.updateDocument(APPWRITE_DATABASE_ID, COLLECTION_CUSTOMERS, customerId, data),
})

const { mutate: uploadImage, isPending: isUploadImagePending } = useMutation({
    mutationKey: ['upload image'],
    mutationFn: (file: File) => storage.createFile(APPWRITE_STORAGE_ID, uuid(), file),
    onSuccess(data) {
        const response = storage.getFileDownload(APPWRITE_STORAGE_ID, data.$id)
...
<template>
    <div class="p-10">
        ...
        <form @submit="onSubmit" class="form">
            ...
            <UIInput placeholder="Откуда пришел?" v-model="fromSource" v-bind="fromSourceAttrs" type="text"
                class="input" />
